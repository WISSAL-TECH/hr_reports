...
class payrollreportexcelwiz(models.TransientModel):
    ...
    def get_item_data(self):
        file_name = _('Livre de paie - %s - %s.xlsx' % (self.from_date, self.date_end))
        fp = BytesIO()
        ...
        worksheet = workbook.add_worksheet('Livre de paie - %s - %s.xlsx' % (self.from_date, self.date_end))
        ...
        if self.from_date and self.date_end:
            ...
            payslip_ids = self.env['hr.payslip'].search(
                [('date_from', '=', self.from_date), ('date_to', '=', self.date_end), ('state', '=', 'done')])
            if payslip_ids:
        ...
        return {
            'name': 'Télécharger le Livre de paie',
            'view_mode': 'form',
